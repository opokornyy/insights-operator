...
func (c *Controller) syncDataGatherCR(stopCh <-chan struct{}) {
	for {
		...
		case jobName := <-c.jobInf.FinishedJob():
			go func() {
				klog.Infof("syncDataGatherCR: JobName finished: %s", jobName)
				...
				if err != nil {
					klog.Errorf("Error while fetching DataGather: %s: %v", dg.Name, err)
					return
				}
				...
				if err != nil {
					klog.Infof("Failed to update progressing conditions: %s", dg.Name)
					return
				}

				klog.Infof("Successfully updated Progressing condition: %s", dg.Name)
			}()
		}
	}
}
...
func (c *Controller) runJobAndCheckResults(ctx context.Context, dataGather *insightsv1alpha2.DataGather, image string) {
	...
	if err != nil {
		klog.Infof("Gathering job failed")
	}
